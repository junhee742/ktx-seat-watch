...
매진된 열차에 취소표가 나오는 순간을 잡는다. 사람이 새로고침을 반복하는 대신 OS 스케줄러(macOS launchd / Linux systemd user timer)가 주기적으로 조회하고, 자리가 열리면 **예약까지 자동으로 마친 뒤 알린다.** 결제는 하지 않는다 — 코레일 구입기한 10분 안에 사용자가 직접 결제해야 한다.
...
- **스케줄러는 Orca·터미널·Claude 세션과 무관하게 돈다.** macOS 는 plist 가 `~/Library/LaunchAgents/` 에, Linux 는 unit 이 `~/.config/systemd/user/` 에 있어 로그인할 때마다 자동으로 뜬다. Linux 에서 로그아웃 후에도 유지하려면 lingering 이 필요하다 — 설치 스크립트가 안내한다.
- **Linux 에서는 텔레그램이 사실상 유일한 알림 통로다.** `osascript` 알림·음성이 없다. 설치 시 미설정이면 경고가 뜬다.
